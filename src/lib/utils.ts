...
export async function findABIForProxy(network: string, proxyAddress: string): Promise<string | null> {
  const api = `https://api${
  ...
  for (let { topic, indexed, dataIndex } of TOPICS_FOR_PROXYS) {
    ...
    if (data.result.length > 0) {
      ...
      const address = indexed ? getAddressByTopic(event, indexed!) : getAddressByData(event, dataIndex!)
      return address
    }
  }
  return null
...
async function getAddressByStorageSlot(proxyAddress: string) {
