...
export async function findABIForProxy(web3: any, network: string, proxyAddress: string): Promise<string | undefined> {
  const api = `https://api${
  ...
  for (let { topic, indexed, dataIndex } of TOPICS_FOR_PROXYS) {
    ...
    if (data.result.length > 0) {
      ...
      address = indexed ? getAddressByTopic(event, indexed!) : getAddressByData(event, dataIndex!)
      if (address) {
        return address
      }
    }
...
async function getAddressByStorageSlot(web3: any, proxyAddress: string): Promise<string | undefined> {
  const res = await fetch(`https://api.etherscan.io/api?module=proxy&action=eth_getStorageAt&address=${proxyAddress}&position=0x7050c9e0f4ca769c69bd3a8ef740bc37934f8e2c036e5a723fd8ee048ed3f8c3&tag=latest`)
